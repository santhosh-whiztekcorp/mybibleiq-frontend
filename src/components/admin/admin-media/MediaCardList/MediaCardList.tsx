...
import { useMediaCardList } from "./MediaCardList.hooks";
...
export function MediaCardList({
  ...
  const { loadMoreRef } = useMediaCardList({
    onLoadMore,
    hasNextPage,
    isFetchingNextPage,
  });
  ...
  if (isLoading && items.length === 0) {
    return (
      <div className="flex flex-col items-center justify-center p-12 min-h-[400px]">
        ...
        <p className="text-[#656A73] font-medium text-sm">Loading media library...</p>
      </div>
    );
  }
  ...
  if (items.length === 0) {
    return (
      <div className="flex flex-col items-center justify-center p-12 min-h-[400px] border-2 border-dashed border-[#E2E8F0] rounded-xl bg-[#F9FAFB]">
        <p className="text-[#656A73] font-bold text-lg">No media items found</p>
        <p className="text-[#83827E] text-sm mt-1 text-center">
          Try adjusting your filters or search terms,
          <br /> or upload your first media item.
        </p>
      </div>
    );
  }
