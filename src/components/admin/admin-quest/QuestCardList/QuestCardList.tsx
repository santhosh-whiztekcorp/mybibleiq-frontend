...
import { useQuestCardList } from "./QuestCardList.hooks";
...
export function QuestCardList({
  ...
  const { loadMoreRef } = useQuestCardList({
    onLoadMore,
    hasNextPage,
    isFetchingNextPage,
  });
  ...
  if (isLoading && items.length === 0) {
    return (
      <div className="flex flex-col items-center justify-center p-12 min-h-[400px]">
        ...
        <p className="text-[#656A73] font-medium text-sm">Loading quests...</p>
      </div>
    );
  }
  ...
  if (items.length === 0) {
    return (
      <div className="flex flex-col items-center justify-center p-12 min-h-[400px] border-2 border-dashed border-[#E2E8F0] rounded-xl bg-[#F9FAFB]">
        <p className="text-[#656A73] font-bold text-lg">No quests found</p>
        <p className="text-[#83827E] text-sm mt-1 text-center">
          Try adjusting your filters or search terms,
          <br /> or create your first quest.
        </p>
      </div>
    );
  }
      ...
      <div ref={loadMoreRef} className="h-10 flex items-center justify-center">
        {isFetchingNextPage && (
          <div className="flex items-center gap-2 text-muted-foreground animate-pulse">
            ...
            <span className="text-sm font-medium">Loading more quests...</span>
          </div>
        )}
      </div>
    </div>
