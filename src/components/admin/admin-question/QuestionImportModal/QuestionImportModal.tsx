...
import { useQuestionImportModal } from "./QuestionImportModal.hooks";
import type { QuestionImportModalProps } from "./QuestionImportModal.types";
import { type ImportReportRow, QUESTION_TYPE_LABELS } from "@/resources/admin-question";
...
export function QuestionImportModal({ open, onOpenChange, onSuccess }: QuestionImportModalProps) {
  const {
    ...
    isUploading,
    isCommitting,
  } = useQuestionImportModal(onSuccess);
      ...
      },
      {
        ...
        cell: ({ row }) => (
          <div className="font-medium whitespace-nowrap">
            {QUESTION_TYPE_LABELS[row.original.data.type] || row.original.data.type || "-"}
          </div>
        ),
      },
      {
        accessorKey: "data.questionText",
        header: "Question",
        cell: ({ row }) => (
          <div className="min-w-[200px] max-w-[400px] truncate text-xs" title={row.original.data.questionText}>
            {row.original.data.questionText || "-"}
          </div>
        ),
      },
      {
        ...
        header: "Errors",
        cell: ({ row }) => (
          <div className="text-red-600 text-[10px] max-w-[200px] leading-tight">
                ...
                <div className="space-y-4">
                  <div>
                    ...
                    <FileUploader
                      ...
                      sampleFileName="question_bulk_upload_template.csv"
                    ...
                    <div className="space-y-3 pt-2">
                      ...
                      <div className="rounded-[10px] border overflow-hidden">
                        <DataTable
                          ...
                          showSearch={true}
                          showPagination={true}
                          searchPlaceholder="Filter by question text..."
                          minWidth="600px"
                          className="bg-white"
                        />
                      </div>
