...
import { useAdminGroupSelector } from "./AdminGroupSelector.hooks";
import type { AdminGroupSelectorProps } from "./AdminGroupSelector.types";

export function AdminGroupSelector({
  value,
  onChange,
  ...
  const {
    ...
    handleToggleGroup,
    handleRemoveGroup,
  } = useAdminGroupSelector(value, onChange);
              ...
              <div className="p-1.5 w-full">
                {allGroups.map((group) => {
                  ...
                  return (
                    ...
                    >
                      ...
                      <div className="flex h-6 w-6 shrink-0 items-center justify-center rounded-full bg-[#ECF5FF] text-[#1447E6] text-[10px] font-bold uppercase">
                        {group.name.charAt(0)}
                      </div>
                      <div className="flex flex-col flex-1 min-w-0">
                        <span className="truncate">{group.name}</span>
                        <span className="text-[10px] text-[#83827E] font-medium truncate">
                ...
                {allGroups.length === 0 && !isLoading && (
                  <div className="py-12 text-center flex flex-col items-center gap-2 animate-in fade-in duration-300">
                    ...
                    <span className="text-xs text-[#83827E] font-bold tracking-tight">No groups found</span>
                  </div>
                )}
