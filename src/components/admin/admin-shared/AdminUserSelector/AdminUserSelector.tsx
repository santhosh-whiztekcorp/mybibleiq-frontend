...
import { useAdminUserSelector } from "./AdminUserSelector.hooks";
import type { AdminUserSelectorProps } from "./AdminUserSelector.types";
...
export function AdminUserSelector({
  ...
  const {
    ...
    handleToggleUser,
    handleRemoveUser,
  } = useAdminUserSelector(value, onChange);
              ...
              <div className="p-1.5 w-full">
                  ...
                  return (
                    ...
                    >
                      ...
                      <Avatar className="h-6 w-6 shrink-0">
                        ...
                        <AvatarFallback className="text-[10px] font-bold bg-[#F3F4F6] text-[#656A73]">
                          {(user.name || user.username || "U").charAt(0).toUpperCase()}
                        </AvatarFallback>
                      </Avatar>
                      <div className="flex flex-col flex-1 min-w-0">
                        <span className="truncate">{user.name}</span>
                        <span className="text-[10px] text-[#83827E] font-medium truncate">@{user.username}</span>
                      </div>
                    </div>
                  );
                })}
                ...
                {allUsers.length === 0 && !isLoading && (
                  <div className="py-12 text-center flex flex-col items-center gap-2 animate-in fade-in duration-300">
                    ...
                    <span className="text-xs text-[#83827E] font-bold tracking-tight">No users found</span>
                  </div>
                )}
