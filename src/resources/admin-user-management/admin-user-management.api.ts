...
export const suspendAdminUser = async (
  ...
  // Convert Date to string if needed for API
  const apiInput: { reason: string; suspendUntil?: string } = {
    reason: input.reason,
    suspendUntil: input.suspendUntil instanceof Date ? input.suspendUntil.toISOString() : input.suspendUntil,
  };
  ...
  if (USE_MOCK_DATA) {
    ...
    return {
      ...
      reason: apiInput.reason || "No reason provided",
      suspendUntil: apiInput.suspendUntil,
    };
  }
...
export const updateAdminUserSettings = async (
  userId: string,
  input: AdminUserSettingsUpdateInput
): Promise<AdminUserSettingsResponse> => {
  if (USE_MOCK_DATA) {
    await mockDelay();
    if (!MOCK_USER_SETTINGS[userId]) {
      MOCK_USER_SETTINGS[userId] = { notifications: true, soundEffects: true };
    }

    if (input.notifications !== undefined) {
      MOCK_USER_SETTINGS[userId].notifications = input.notifications;
    }

    if (input.soundEffects !== undefined) {
      MOCK_USER_SETTINGS[userId].soundEffects = input.soundEffects;
    }

    return MOCK_USER_SETTINGS[userId];
  }

  const response = await apiClient.patch<ApiResponseEnvelope<AdminUserSettingsResponse>>(
    endpoints.userManagementAdmin.updateSettings(userId),
    input
  );
  return unwrapApiResponse(response);
};
...
export const updateAdminUserSpiritFood = async (
  userId: string,
  input: AdminUserSpiritFoodUpdateInput
): Promise<AdminUserSpiritFoodResponse> => {
  if (USE_MOCK_DATA) {
    await mockDelay();
    if (!MOCK_USER_SPIRIT_FOOD[userId]) {
      MOCK_USER_SPIRIT_FOOD[userId] = {
        savedVerses: 0,
        deliveryTime: "08:00",
        deliveryMethods: [],
    ...
    if (input.deliveryTime !== undefined) {
      MOCK_USER_SPIRIT_FOOD[userId].deliveryTime = input.deliveryTime;
    }

    if (input.deliveryMethods !== undefined) {
      MOCK_USER_SPIRIT_FOOD[userId].deliveryMethods = input.deliveryMethods;
    }

    if (input.timezone !== undefined) {
      MOCK_USER_SPIRIT_FOOD[userId].timezone = input.timezone;
    }

    if (input.isEnabled !== undefined) {
      MOCK_USER_SPIRIT_FOOD[userId].isEnabled = input.isEnabled;
    }

    return MOCK_USER_SPIRIT_FOOD[userId];
  }

  const response = await apiClient.patch<ApiResponseEnvelope<AdminUserSpiritFoodResponse>>(
    endpoints.userManagementAdmin.updateSpiritFood(userId),
    input
  );
  return unwrapApiResponse(response);
};
